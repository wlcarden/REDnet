...
**Axis 3 — data-at-rest.** Short **retention** (the single biggest honeypot lever); **LUKS** full-disk encryption (protects a powered-off/decommissioned disk only — useless against a running box or compelled operator); **encrypted off-box backups** with the key _not_ stored on the server; minimal logging + short log retention; no third-party log services. Irreducible residual: while running, the DB holds the membership graph + the unencrypted who/when/which-room envelope of every message + fresh client IPs (Synapse retains IPs for up to 1 day via `user_ips_max_age`; MAS IPs are scrubbed hourly by `scrub-metadata.sh`; there is no IP-logging off-switch, only bounded windows) + receipts/typing (cannot be disabled server-side). That is the honeypot tax — bounded here, never erased; which is exactly why dangerous content goes to Tier 2.
...
**Recovery — phased approach.** Matrix accounts live on the _server_, not the device; phone and web are two devices under one account that share keys via cross-signing. So device-loss is only catastrophic for a _single-device_ user with no recovery key.

- **Phase 0 (shipped): accept-loss + device redundancy.** Nudge users to stay logged in on web; a second verified session survives a lost phone. Lost your only device → re-onboard as a fresh pseudonym; short retention (§10) makes history loss trivial.
- **Phase 1 (built, verified): passphrase recovery.** A diceware passphrase (generated at onboarding, stored by the member) restores cross-signing identity + key backup history. Proven in browser E2E.
- **Phase 2 (crypto built, coordination tooling not yet built): governance-gated escrow.** Shamir M-of-N splits + ECIES per-member encryption + HKDF-SHA256 + AES-256-GCM. **Default is passphrase + M-of-N** (member passphrase ANDs with the quorum, capping blast radius per member). See [RECOVERY.md](RECOVERY.md) for the full construction, threat analysis, and the decision to default to passphrase mode.
- **What "no operator-held escrow" means in practice:** the escrow store is encrypted such that no single party (not the server, not any individual moderator) can recover keys. A seized core yields ciphertext; recovering a member requires both the quorum AND that member's passphrase.

The card carries a dormant per-account **claim secret** (a low-sensitivity claim token, _not_ a decryption key); forward-compat hooks are baked into v1 since they can't be retrofitted onto distributed cards.
...
**Verified 2026-07-21 (current-sources review; still DEFERRED).** Content-E2EE is genuine — per-sender AES-GCM frame encryption keyed over Matrix's audited Olm, so the LiveKit SFU relays ciphertext only — and Element declares the stack "production ready." But there is **no independent audit** of the Element Call E2EE implementation, the underlying SFrame construction has **no per-sender authentication** (RFC 9605 — an in-call member can forge another's media), and its forward-secrecy claim did not hold up under verification. The media-node metadata exposure (participant IPs + co-presence + timing) is **irreducible** — WebRTC is UDP and Tor is TCP-only, so calls cannot ride Tor — and is comparable to Signal's own group-call exposure. Two deployment landmines were found and are now flagged in the module and in [deploy/README.md](deploy/README.md) ("Before enabling"): the pinned `lk-jwt-service` shipped an insecure `*`-wildcard default (fixed upstream in v0.5.0) and the well-known discovery format is likely stale. Net: the defer decision stands — enable only as a hardened, opt-in, separate-box module.
...
**In-client governance (gov bot):** a non-E2EE command room (`#gov-bot`) hosts `!gov` commands for confirm/revoke/role/audit/status, eliminating SSH for routine governance. `!gov status` probes Synapse liveness (via `/_matrix/client/versions`), reports community membership count, vouch index, active report DMs, and alert count — giving the operator situational awareness from their phone. Every high-value governance action is now surfaced as in-context Element UI in the soft-fork (a governance menu, right-click "Report to organizers", "Manage member" for role/revoke, approve/deny, confirm-vouch), with `!gov`/`!report` as the underlying command transport. The bot writes to `vouch.jsonl` (canonical audit trail) but does not post to E2EE rooms it cannot encrypt for. For member-facing reporting, the bot creates an unencrypted DM room with each member on join; members use the client's "Report to organizers" action (or `!report` by hand) there instead of needing `#gov-bot` access, and reports are forwarded to `#gov-bot` for operators. The reporter's identity is visible to operators but not to the reported user.

**Community management (room/space creation):** room creation is locked to the system accounts by a Synapse module — members' clients can still create DMs, but every shared room or space is born through the gov bot: E2EE with no off switch, linked into the space hierarchy, logged to the audit trail. Members request rooms privately (a "Request a room or space" button, or `!gov request` in their bot DM); organizers approve/deny from an in-client governance menu (or in `#gov-bot`), and the requester becomes the new room's moderator (PL 50) — approve once, then hands off. Creators choose a visibility tier per room: **open** (any member of the parent space joins freely), **knock** (visible, join by moderator approval — the default, fails closed), or **private** (invite-only; optionally unlisted so even the room's name never appears in the hierarchy). Full spec: `deploy/COMMUNITY-MANAGEMENT.md`.
